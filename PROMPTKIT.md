...
- **Domain / Purpose**: Rock-solid double-entry bookkeeping engine for freelancers and small businesses (Phase 0: manual CoA + journal + Trial Balance / P&L, local-only)
...
- **State & Data Fetching**: Server Actions + Prisma directly (no separate public API in Phase 0)
- **Backend & Database**: SQLite (`ledger.db`), Prisma ORM
...
- **Install**: `bun install`
- **Dev Server**: `bun dev` (local-only, Phase 0)
- **Unit Tests**: `bun test`
- **E2E Tests**: `bun test:e2e` (deferred — no harness yet)
- **Typecheck**: `bunx tsc --noEmit`
- **Lint & Format**: `bun run lint` (to be defined at scaffold)

---

## 4. Monorepo & Workspace Topology (If Applicable)
N/A (Standalone Repository — single Next.js app + local SQLite, Phase 0)
