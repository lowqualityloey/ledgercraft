...
- **Technical Debt & Risks**:
  ...
  - **Platform API token leak — OPEN, high severity (2026-09-22)**: `.env` line 21 holds a live Turso **platform (control-plane)** credential. Probed to confirm its nature rather than assume it: the libSQL endpoint returns 401 (so it is not a database token) while `api.turso.tech/v1/organizations` returns 200 (so it is a platform token that can mint further database tokens and manage the org). It is the token that passed through chat. It is 194 chars and does **not** match the CLI's own 673-char token, and `turso auth api-tokens list` shows only `dmturbocli`, so it is most likely a legacy token the CLI cannot enumerate — revoke it in the Turso dashboard. Verify the kill by re-probing line 21 against `api.turso.tech` (expect 401), then delete that line from `.env`.
...
2. ~~Mint a fresh database token and update Vercel + `.env`~~ — **done 2026-09-22**: new 348-char token live in Vercel Production and `.env` line 22, verified against the hosted DB; the Preview database env gap was also filled. **Still open, in this order:** (a) revoke the leaked platform token in the Turso dashboard and confirm `.env` line 21 starts returning 401, then delete that line; (b) decide whether to run the full `turso db tokens invalidate` rotation, which kills the old database token but also the current one and costs a short production window (see §5).
