...
- **Current Milestone / Epic**: Milestones 1–7 shipped (2026-09-17); Vercel build repaired (2026-09-18) and the ledger moved onto a hosted libSQL database (Turso), now committed; PromptKit OS engine rebaselined as a pinned submodule with the generated agent config committed (2026-09-22) → Next: polish / release TBD (Turso credential rotation completed 2026-09-23 — every pre-rotation database token verified dead)
...
| 2026-09-23 (Turso revoke path) | ~10 turns | host telemetry unavailable | not measured | Found and executed the real revoke path for database tokens (platform API database-level rotate), proven on a throwaway DB before production; 1 database token rotated, 1 scoped API token minted then revoked, 1 probe DB created then destroyed, 1 Vercel env write, 1 redeploy; no application code changed and no test suite run, so no green-gate claim |
