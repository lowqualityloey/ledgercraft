...
- **Technical Debt & Risks**:
  ...
  - **Database token rotation — PARTIAL (2026-09-22)**: a fresh 348-char token is live in Vercel Production and in `.env` line 22, verified by querying the hosted DB (`Account` = 15). The **old database token remains valid**. The only CLI mechanism, `turso db tokens invalidate`, rotates the signing keys, and Turso's docs confirm it invalidates *all* existing tokens — including a token minted moments earlier — so "mint then invalidate" cannot work. Full invalidation was deliberately deferred to avoid a production window. Correct order when run: `invalidate` → mint → wire Vercel (Production **and** Preview) → update `.env` → redeploy → verify. Authenticated routes 500 during the wiring window; `/login` is static and unaffected.
...
2. ~~Mint a fresh database token and update Vercel + `.env`~~ — **done 2026-09-22**: new 348-char token live in Vercel Production and `.env` line 22, verified against the hosted DB; the Preview database env gap was also filled. ~~(a) revoke the leaked platform token~~ — **done 2026-09-22**: it was `dmturbocli`; revoked, confirmed dead by probing `.env` line 21 (200 → 401), and the line deleted. **Still open:** (b) decide whether to run the full `turso db tokens invalidate` rotation, which kills the old database token but also the current one and costs a short production window (see §5). Note the API-token list is now empty, so any future Turso automation needs a freshly minted token.
