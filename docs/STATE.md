...
- **Technical Debt & Risks**:
  ...
  - **Database token rotation — running app rotated, BUT OLD TOKENS COULD NOT BE REVOKED (2026-09-22)**: Production, Preview and `.env` line 22 now all hold the same fresh 348-char token (`cf702c61ef72`), verified against the hosted DB (`Account` = 15) and by smoke-testing the redeployed app (`/` 307, `/login` 200, `/journal` and `/trial-balance` 307 → `/login` with a forged cookie, proving the DB read succeeds rather than merely that middleware redirected). **The invalidation step does not work, so every earlier token is still valid** — see the next entry. There was never a production outage: the initial `turso db tokens invalidate ledgercraft` was rejected before doing anything, and the redeploy happened while the old token was still live.
  - **`turso … tokens invalidate` is INEFFECTIVE for this database — observed, and it contradicts Turso's docs (2026-09-22)**: `turso db tokens invalidate ledgercraft` is rejected outright for a grouped database (`Error: database ledgercraft is part of group default, use turso group tokens invalidate <group-name> instead`). The group form, `turso group tokens invalidate default -y`, prints `Success! Tokens invalidated successfully.` but retires nothing: two probe tokens minted *immediately before* the call still authenticated afterwards (`200`), and the pre-existing token `c35bf3cb461c` survived at +0s, +20s, +40s and again at +45s. Turso's docs claim the opposite ("all existing tokens will no longer be valid and will need to be regenerated"), so this is a docs-versus-behaviour gap, not a misuse of the command. **Consequence:** database tokens for this database appear non-revocable from the CLI, so every mint adds a permanently valid credential — seven now exist (`d63c8255868a` from the 2026-09-18 migration, plus `c35bf3cb461c`, `0b2f005cb47c`, `8784ed46c478`, `f6f5d5ed4992`, `f64e3de575cc`, `cf702c61ef72`). The database now therefore has a **wider** live-credential surface than before rotation began, which is the opposite of the intent. **Untested mitigations, in order of preference:** a per-token revoke via the Turso dashboard or the platform API; Turso support; or recreating the database (destructive — requires a dump/restore and is not a small change). Do not assume a future `invalidate` works — re-probe before relying on it, and note that re-probing costs a mint that cannot be withdrawn.
...
- **Running total**: 7 sessions logged — measured figures unavailable; estimates only where the host exposed them.
